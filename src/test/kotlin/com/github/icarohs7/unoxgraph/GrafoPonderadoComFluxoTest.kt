...
import com.github.icarohs7.unoxgraph.extensoes.comFluxo
...
class GrafoPonderadoComFluxoTest : StringSpec() {
	private lateinit var grafo: Grafo.PonderadoComFluxo   //Grafo1
	private lateinit var grafo2: Grafo.PonderadoComFluxo  //Grafo2
	private lateinit var grafoDirecionado: Grafo.PonderadoComFluxo //Grafo não direcionado
	...
	init {
		...
		"Deve excluir uma aresta" {
			val g3 = Grafo.PonderadoComFluxo.ofASize(3, direcionado = false)
			g3 += Aresta(0, 1, 1.0).comFluxo(1.0)
			g3 += Aresta(1, 2, 2.0).comFluxo(2.0)
			grafo2 -= 2 para 0
		...
		"Deve ser construído de forma idiomática, direcionado" {
			val gi = Grafo.PonderadoComFluxo.withThePath(0, 1, 2, 0, direcionado = false)
			...
			gi.matrizFluxo[0][1] = 1.0
			gi.matrizFluxo[1][0] = 1.0
			gi.matrizFluxo[1][2] = 2.0
			gi.matrizFluxo[2][1] = 2.0
			gi.matrizFluxo[2][0] = 3.0
			gi.matrizFluxo[0][2] = 3.0
			gi shouldBe grafo
		}
		
		"Deve ser construído de forma idiomática, não direcionado" {
			val gdi = Grafo.PonderadoComFluxo.withThePath(0, 1, 2, 0, direcionado = true)
			gdi.matrizAdjacencia[0][1] = 1.0
			gdi.matrizAdjacencia[1][2] = 2.0
			gdi.matrizAdjacencia[2][0] = 3.0
			gdi.matrizFluxo[0][1] = 1.0
			gdi.matrizFluxo[1][2] = 2.0
			gdi.matrizFluxo[2][0] = 3.0
			gdi shouldBe grafoDirecionado
		}
		
		"Deve realizar uma copia fiel de si mesmo" {
			val copia = grafo.copy()
	...
	override fun beforeTest(description: Description) {
		grafo = Grafo.PonderadoComFluxo.ofASize(3, direcionado = false)
		grafo += Aresta(0, 1, 1.0).comFluxo(1.0)
		grafo += Aresta(1, 2, 2.0).comFluxo(2.0)
		grafo += Aresta(2, 0, 3.0).comFluxo(3.0)
		
		grafo2 = Grafo.PonderadoComFluxo.ofASize(3, direcionado = false)
		grafo2 += Aresta(0, 1, 1.0).comFluxo(1.0)
		grafo2 += Aresta(1, 2, 2.0).comFluxo(2.0)
		grafo2 += Aresta(2, 0, 3.0).comFluxo(3.0)
		...
		grafoDirecionado = Grafo.PonderadoComFluxo.ofASize(3, direcionado = true)
		grafoDirecionado += Aresta(0, 1, 1.0).comFluxo(1.0)
		grafoDirecionado += Aresta(1, 2, 2.0).comFluxo(2.0)
		grafoDirecionado += Aresta(2, 0, 3.0).comFluxo(3.0)
